...
export const createProvider = async (req, res) => {
    ...
    const result = await pool.query('INSERT INTO providers (name, email, phone, tax_id) VALUES ($1, $2, $3, $4) RETURNING *', [name, email, phone, taxId]);
...
export const updateProvider = async (req, res) => {
    ...
    const result = await pool.query('UPDATE providers SET name = $1, email = $2, phone = $3, tax_id = $4 WHERE id = $5 RETURNING *', [name, email, phone, taxId, Number(id)]);
    if (result.rows.length === 0)
        return res.status(404).json({ message: 'Proveedor no encontrado' });
    res.json(result.rows[0]);
};
