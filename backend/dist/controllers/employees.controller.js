...
export const createEmployee = async (req, res) => {
    ...
    const result = await pool.query('INSERT INTO users (name, email, password, role_id) VALUES ($1, $2, $3, $4) RETURNING *', [name, email, hashedPassword, roleId]);
    res.status(201).json(result.rows[0]);
};
export const getEmployees = async (_, res) => {
    const result = await pool.query(`SELECT users.*, roles.name as role_name 
     FROM users 
...
export const updateUser = async (req, res) => {
    ...
    const result = await pool.query('UPDATE users SET name = $1, email = $2, role_id = $3 WHERE id = $4 RETURNING *', [name, email, roleId, Number(req.params.id)]);
    res.json(result.rows[0]);
};
