...
export const createRole = async (req, res) => {
    ...
    try {
        const roleResult = await pool.query('INSERT INTO roles (name) VALUES ($1) RETURNING *', [name]);
        const role = roleResult.rows[0];
        for (const action of permissions) {
            await pool.query('INSERT INTO permissions (role_id, action) VALUES ($1, $2)', [role.id, action]);
        }
...
export const updateRole = async (req, res) => {
    ...
    try {
        ...
        for (const action of permissions) {
            await pool.query('INSERT INTO permissions (role_id, action) VALUES ($1, $2)', [id, action]);
        }
...
export function getRoleById(arg0, getRoleById) {
