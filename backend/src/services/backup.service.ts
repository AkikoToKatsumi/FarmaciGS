...
class BackupService {
   ...
   */
  async createBackup(userId?: number): Promise<BackupResult> {
    if (this.isRunning) {
   ...
   */
  async restoreBackup(options: RestoreOptions, userId?: number): Promise<BackupResult> {
    const startTime = Date.now();
   ...
   */
  async deleteBackup(filename: string, userId?: number): Promise<boolean> {
    try {
      const filepath = path.join(this.config.backupDir, filename);
    ...
    cron.schedule('0 1 * * 0', async () => {
      ...
      if (result.success && result.filename) {
        ...
        await emailService.sendSystemNotification(
          process.env.ADMIN_EMAIL || 'admin@farmacia.com',
          'Respaldo Semanal Completado',
          `El respaldo semanal se ha completado exitosamente.\nArchivo: ${result.filename}\nTamaño: ${this.formatFileSize(result.size || 0)}`
        );
      }
    });
  }
   ...
   */
  private getAgeString(date: Date): string {
    const now = new Date();
